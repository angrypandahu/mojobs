...
class RegisterController extends AbstractS2UiController {
    ...
    def forgotPassword(ForgotPasswordCommand forgotPasswordCommand) {
        ...
        def user = com.domain.auth.User.findByEmail(forgotPasswordCommand.email)
        if (!user) {
            forgotPasswordCommand.errors.rejectValue 'email',
                    'spring.security.ui.forgotPassword.email.notFound'
            return [forgotPasswordCommand: forgotPasswordCommand]
        }
...
class RegisterCommand implements CommandObject {
    ...
    static constraints = {
        ...
        email email: true, validator: { value, command ->
            ...
            if (com.domain.auth.User.countByEmail(value) > 0) {
